...
# Input optimal features for alignment data (as a list) returned from GetOptimalFeatures.py 
optimal_features = []
...
def create_vector(split_sequence, position_map):
    ...
    # Function to convert full-length vectors to vectors of only optimal features, given optimal feature positions
def create_optimal_feature_vector(split_vector, features):
    optimal_vector = []
    for position in features:
        optimal_vector.append(split_vector[position - 1])
    return optimal_vector


# Converting sequences in alignment to optimal feature vector representations
sequences = []
descriptions = []
...
optimal_feature_vectors = []
for sequence in split_sequences_vectors:
    optimal_feature_vectors.append(create_optimal_feature_vector(sequence, optimal_features))
...
for j in range(len(split_sequences_vectors)):
    if 'C4_' in descriptions[j]:
        c4_vectors.append(optimal_feature_vectors[j])
    if 'C3_' in descriptions[j]:
        c3_vectors.append(optimal_feature_vectors[j])
